 ...
 * dissertation [1]. There are some changes and assumptions: (1) This
 * implementation does not use the mentioned Bitbrains distribution for
 * generating random values of the virtual networks. (2) This implementation
 * does not feature any special properties like master-failover-servers, low
 * latency links, or high performance computing nodes. (3) Every generated
 * workload (virtual network) is a one tier network with (star topology), but
 * every server of a virtual network is configured exactly the same way.
 *
...
public abstract class AMdvneAdaptedScenario implements IScenario {
	...
	/**
	 * Returns the next random integer of the pseudo random number generator in
	 * (min, max).
	 *
	...
	/**
	 * Sets one instance of a virtual network up (according to the configuration and
	 * to pseudo random values from all attribute ranges above).
	 *
	...
	protected void virtualSetup(final String virtualNetworkId) {
		...
		final OneTierConfig virtualConfig = new OneTierConfig(numberOfServers, 1, false, cpu, memory, storage,
				bandwidth);
